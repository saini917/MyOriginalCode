...
public class NRDA_BOD_SC_01_TC_44 extends CommonLibrary
{
	...
	private WebElement selctaction;
	
	public void selectAction() throws InterruptedException
	{
		wait.until(ExpectedConditions.visibilityOf(selctaction));
		Assert.assertTrue(selctaction.isDisplayed());
		HighlightOnElement(selctaction);
		Assert.assertTrue(selctaction.isEnabled());
		SelectVisibleText(selctaction,"Forward");
		waitForSomeTime();
	}
	
	/****************************************************************************************************/
	/*
	 * This is the xpath of the WebElement "Select action name".
	 * @author deepak.saini
	 * @since 2018-02-16
	 */
	/*****************************************************************************************************/
	@FindBy(xpath="//label[contains(text(),'Select Action')]/../../../following-sibling::td/select/../following-sibling::td/span/table/tbody/tr/td/select")
	private WebElement selctname;
	
	public void selectName() throws InterruptedException
	{
		wait.until(ExpectedConditions.visibilityOf(selctaction));
		Assert.assertTrue(selctname.isDisplayed());
		HighlightOnElement(selctname);
		Assert.assertTrue(selctname.isEnabled());
		SelectVisibleText(selctname,"R.P  Shrivastava (EE)");
		waitForSomeTime();
	}
