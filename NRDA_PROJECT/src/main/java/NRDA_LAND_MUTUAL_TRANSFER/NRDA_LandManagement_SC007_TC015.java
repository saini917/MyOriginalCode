...
public class NRDA_LandManagement_SC007_TC015 extends CommonLibrary{
	...
	@FindBy(xpath="//span[contains(text(),'View')][2]")
	private WebElement view1;

	public void clickOnView1() throws InterruptedException{
		JavascriptExecutor js = (JavascriptExecutor) driver;
		js.executeScript("arguments[0].scrollIntoView();",view1);
		
		wait.until(ExpectedConditions.visibilityOf(view1));
		Assert.assertTrue(view1.isDisplayed());
		HighlightOnElement(view1);
		Assert.assertTrue(view1.isEnabled());
		wait.until(ExpectedConditions.elementToBeClickable(view1));
		view1.click();
		waitForPageLoad();
		waitForDomElement();
	}
